...
void SysLog::msg(Log::Level _ll, const std::string& _msg, const std::string&)
{
	const std::string prefix = (Context::get().empty() ? "" : "[" + Context::get() + "] -- ");
	if (_ll == Log::LL_TRACE)
	{
	  _ll = Log::LL_DEBUG;
	}
	syslog(_ll | ((LOG_FAC(LOG_LOCAL0) + facility) << 3), "%s", (prefix + _msg).c_str());
}
