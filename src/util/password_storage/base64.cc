        ...
        int size_of_data)
{
    ...
    std::copy(Base64EncodeIterator(data),
              Base64EncodeIterator(data + size_of_data),
...
BinaryData::BinaryData()
    : size_of_raw_binary_data_()
{ }

BinaryData::BinaryData(const PasswordStorage::Base64EncodedData& _src)
{
    RawData raw = get_raw_binary_data_from_base64_encoded_data(_src.get_base64_encoded_data());
    raw_binary_data_ = std::move(raw.data);
...
{
    BinaryData result;
    result.raw_binary_data_ = std::move(_raw_binary_data);
    result.size_of_raw_binary_data_ = _size_of_raw_binary_data;
    return result;
}

const unsigned char* BinaryData::get_raw_binary_data()const
{
    return raw_binary_data_.get();
}
