...
public:
  ...
  void setAlias(const std::string& _alias) { alias = _alias; }
  std::string str() const { return name + " " + alias; }
...
public:
  ...
  std::string getFunct() const { return funct; }
  bool hasFunct() const { return !(funct.empty()); }
  ...
  {
    const bool f = hasFunct();
    const bool c = !cast_to.empty();
...
public:
  Condition(Column _l, const std::string& _op, Column _r)
      : left(_l),
        ...
        right(_r) { }
  Condition() { }
  const Column* getLeft() const { return &left; }
  const Column* getRight() const { return &right; }
  std::string getOperator() const { return op; }
  std::string str() const { return left.str() + op + right.str(); }
...
public:
  ...
  std::string str() const
  {
    return left.getTable().str() + join_type + right.getTable().str() + " ON (" + Condition::str() + ")";
  }
  std::string str_without_left() const
  {
    return join_type + right.getTable().str() + " ON (" + Condition::str() + ")";
  }
