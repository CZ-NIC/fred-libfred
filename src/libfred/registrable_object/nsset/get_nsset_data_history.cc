        ...
        T get_object_id_rule)
{
    ...
    for (std::size_t idx = 0; idx < dbres.size(); ++idx)
    {
        if (idx == 0)
        {
            history.object_uuid = dbres[0][0].as<NssetUuid>();
        }
        typename NssetDataHistory::Record record;
        record.history_uuid = dbres[idx][1].as<NssetHistoryUuid>();
        record.valid_from = static_cast<typename NssetDataHistory::TimePoint>(dbres[idx][2]);
        ...
        {
            history.valid_to = static_cast<typename NssetDataHistory::TimePoint>(dbres[idx][3]);
        }
...
GetNssetDataHistoryById::GetNssetDataHistoryById(unsigned long long nsset_id)
    : nsset_id_(nsset_id)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    return get_nsset_data_history(ctx, range, OperationById(nsset_id_));
}

GetNssetDataHistoryByHandle::GetNssetDataHistoryByHandle(const std::string& nsset_handle)
    : handle_(nsset_handle)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    return get_nsset_data_history(ctx, range, OperationByHandle(handle_));
}

GetNssetDataHistoryByUuid::GetNssetDataHistoryByUuid(const NssetUuid& nsset_uuid)
    : uuid_(nsset_uuid)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    return get_nsset_data_history(ctx, range, OperationByUuid(uuid_));
}
