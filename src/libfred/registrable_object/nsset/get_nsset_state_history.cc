        ...
        T get_object_id_rule)
{
    ...
    const auto upper_limit = static_cast<typename NssetStateHistory::TimePoint>(dbres[0][2]);
    const bool object_has_state_flags = (1 < dbres.size()) || !dbres[0][6].isnull();
    if (!object_has_state_flags)
    {
        NssetStateHistory history;
        typename NssetStateHistory::Record record;
        record.valid_from = static_cast<typename NssetStateHistory::TimePoint>(dbres[0][1]);
        history.timeline.push_back(record);
        history.valid_to = upper_limit;
        return history;
    }
    typename NssetStateHistory::Record record_on_begin;
    struct StateAction
    {
        NssetState state_begin;
        NssetState state_end;
    };
    using StateChanges = std::map<typename NssetStateHistory::TimePoint, StateAction>;
    StateChanges state_changes;
    for (std::size_t idx = 0; idx < dbres.size(); ++idx)
    {
        const bool collect_state_on_begin = static_cast<bool>(dbres[idx][3]);
        const auto valid_from = static_cast<typename NssetStateHistory::TimePoint>(dbres[idx][4]);
        ...
        {
            const auto valid_to = static_cast<typename NssetStateHistory::TimePoint>(dbres[idx][5]);
    ...
    {
        const auto lower_limit = static_cast<typename NssetStateHistory::TimePoint>(dbres[0][1]);
...
}//namespace LibFred::RegistrableObject::Nsset::{anonymous}

GetNssetStateHistoryById::GetNssetStateHistoryById(unsigned long long nsset_id)
    : nsset_id_(nsset_id)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    return get_nsset_state_history(ctx, range, OperationById(nsset_id_));
}

GetNssetStateHistoryByHandle::GetNssetStateHistoryByHandle(const std::string& _nsset_handle)
    : handle_(_nsset_handle)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    return get_nsset_state_history(ctx, range, OperationByHandle(handle_));
}

GetNssetStateHistoryByUuid::GetNssetStateHistoryByUuid(const NssetUuid& _nsset_uuid)
    : uuid_(_nsset_uuid)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    {
    public:
        explicit OperationByUuid(const NssetUuid& uuid)
            : uuid_(uuid) { }
        std::string operator()(Database::query_param_list& params) const
        {
    ...
    private:
        const NssetUuid uuid_;
    };
    return get_nsset_state_history(ctx, range, OperationByUuid(uuid_));
}
