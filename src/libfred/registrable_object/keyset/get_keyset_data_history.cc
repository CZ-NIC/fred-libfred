...
template <typename T>
KeysetDataHistory get_keyset_data_history(
        ...
        T get_object_id_rule)
{
    ...
    for (std::size_t idx = 0; idx < dbres.size(); ++idx)
    {
        if (idx == 0)
        {
            history.object_uuid = dbres[0][0].as<KeysetUuid>();
        }
        typename KeysetDataHistory::Record record;
        record.history_uuid = dbres[idx][1].as<KeysetHistoryUuid>();
        record.valid_from = static_cast<typename KeysetDataHistory::TimePoint>(dbres[idx][2]);
        ...
        {
            history.valid_to = static_cast<typename KeysetDataHistory::TimePoint>(dbres[idx][3]);
        }
...
GetKeysetDataHistoryById::GetKeysetDataHistoryById(unsigned long long keyset_id)
    : keyset_id_(keyset_id)
{
}

GetKeysetDataHistoryById::Result GetKeysetDataHistoryById::exec(
        OperationContext& ctx,
        const HistoryInterval& range) const
{
    ...
    return get_keyset_data_history(ctx, range, OperationById(keyset_id_));
}

GetKeysetDataHistoryByHandle::GetKeysetDataHistoryByHandle(const std::string& keyset_handle)
    : handle_(keyset_handle)
{
}

GetKeysetDataHistoryByHandle::Result GetKeysetDataHistoryByHandle::exec(
        OperationContext& ctx,
        const HistoryInterval& range) const
{
    Database::query_param_list params(Conversion::Enums::to_db_handle(object_type));
    ...
    return get_keyset_data_history(ctx, range, OperationByHandle(handle_));
}

GetKeysetDataHistoryByUuid::GetKeysetDataHistoryByUuid(const KeysetUuid& keyset_uuid)
    : uuid_(keyset_uuid)
{
}

GetKeysetDataHistoryByUuid::Result GetKeysetDataHistoryByUuid::exec(
        OperationContext& ctx,
        const HistoryInterval& range) const
{
    Database::query_param_list params(Conversion::Enums::to_db_handle(object_type));
    ...
    return get_keyset_data_history(ctx, range, OperationByUuid(uuid_));
}
