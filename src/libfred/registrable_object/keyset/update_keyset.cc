...
unsigned long long UpdateKeyset::exec(const OperationContext& ctx)
{
    ...
    try
    {
        ...
        try
        {
            history_id = LibFred::UpdateObject(handle_, "keyset", registrar_
        ...
        if (!rem_tech_contact_.empty())
        {
            ...
            for (std::vector<std::string>::iterator i = rem_tech_contact_.begin(); i != rem_tech_contact_.end(); ++i)
            {
                ...
                const Database::Result keyset_del_res = ctx.get_conn().exec_params(sql_i.str(), params_i);
        ...
        if (!rem_dns_key_.empty())
        {
            ...
            {
                const Database::Result rem_dns_key_res = ctx.get_conn().exec_params(
