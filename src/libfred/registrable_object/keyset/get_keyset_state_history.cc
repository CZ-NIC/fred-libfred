        ...
        T get_object_id_rule)
{
    ...
    const auto upper_limit = static_cast<typename KeysetStateHistory::TimePoint>(dbres[0][2]);
    const bool object_has_state_flags = (1 < dbres.size()) || !dbres[0][6].isnull();
    if (!object_has_state_flags)
    {
        KeysetStateHistory history;
        typename KeysetStateHistory::Record record;
        record.valid_from = static_cast<typename KeysetStateHistory::TimePoint>(dbres[0][1]);
        history.timeline.push_back(record);
        history.valid_to = upper_limit;
        return history;
    }
    typename KeysetStateHistory::Record record_on_begin;
    struct StateAction
    {
        KeysetState state_begin;
        KeysetState state_end;
    };
    using StateChanges = std::map<typename KeysetStateHistory::TimePoint, StateAction>;
    StateChanges state_changes;
    for (std::size_t idx = 0; idx < dbres.size(); ++idx)
    {
        const bool collect_state_on_begin = static_cast<bool>(dbres[idx][3]);
        const auto valid_from = static_cast<typename KeysetStateHistory::TimePoint>(dbres[idx][4]);
        ...
        {
            const auto valid_to = static_cast<typename KeysetStateHistory::TimePoint>(dbres[idx][5]);
    ...
    {
        const auto lower_limit = static_cast<typename KeysetStateHistory::TimePoint>(dbres[0][1]);
...
}//namespace LibFred::RegistrableObject::Keyset::{anonymous}

GetKeysetStateHistoryById::GetKeysetStateHistoryById(unsigned long long keyset_id)
        ...
        const HistoryInterval& range) const
{
    ...
    return get_keyset_state_history(ctx, range, OperationById(keyset_id_));
}

GetKeysetStateHistoryByHandle::GetKeysetStateHistoryByHandle(const std::string& _keyset_handle)
    : handle_(_keyset_handle)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    {
    public:
        explicit OperationByHandle(const std::string& _handle)
            : handle_(_handle) { }
        std::string operator()(Database::query_param_list& params) const
        {
    ...
    return get_keyset_state_history(ctx, range, OperationByHandle(handle_));
}

GetKeysetStateHistoryByUuid::GetKeysetStateHistoryByUuid(const KeysetUuid& _keyset_uuid)
    : uuid_(_keyset_uuid)
{
}
        ...
        const HistoryInterval& range) const
{
    ...
    {
    public:
        explicit OperationByUuid(const KeysetUuid& uuid)
            : uuid_(uuid) { }
        std::string operator()(Database::query_param_list& params) const
        {
    ...
    private:
        const KeysetUuid uuid_;
    };
    return get_keyset_state_history(ctx, range, OperationByUuid(uuid_));
}
