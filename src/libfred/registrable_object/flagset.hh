...
struct FlagSet
{
    ...
    public:
        ...
        Array& set_all(bool value)noexcept;
        Array& set_all()noexcept;
        Array& reset_all()noexcept;
        Array& flip_all()noexcept;
    ...
    private:
        ...
        constexpr Array(const ItemsStorage& src);
        ...
        struct Iterate
        {
            ...
            template <typename V>
            static void over(const V&, const Array&) { }
            template <typename V>
        ...
        {
            template <typename V>
            static void over(V& visitor, const Array<o, Flags...>& status)
            {
                if (visitor.template visit<F, idx>(status) == Visiting::can_continue)
                {
                    Iterate<idx + 1, Fs...>::over(visitor, status);
                }
            }
            template <typename V>
            static void over(const V& visitor, const Array<o, Flags...>& status)
            {
                if (visitor.template visit<F, idx>(status) == Visiting::can_continue)
                {
                    Iterate<idx + 1, Fs...>::over(visitor, status);
            ...
            template <typename M>
            static void over(M& mutator, Array<o, Flags...>& status)
            {
                if (mutator.template visit<F, idx>(status) == Visiting::can_continue)
                {
                    Iterate<idx + 1, Fs...>::over(mutator, status);
                }
            }
            template <typename M>
            static void over(const M& mutator, Array<o, Flags...>& status)
            {
                if (mutator.template visit<F, idx>(status) == Visiting::can_continue)
                {
                    Iterate<idx + 1, Fs...>::over(mutator, status);
            ...
            {
                if (visitor.template visit<F, idx>() == Visiting::can_continue)
                {
        ...
        friend Array operator&(const Array& lhs, const Array& rhs)noexcept
        {
            return Array(lhs.flags_ & rhs.flags_);
        }
        friend Array operator|(const Array& lhs, const Array& rhs)noexcept
        {
            return Array(lhs.flags_ | rhs.flags_);
