...
Database::ParamQuery InfoContact::make_query(const std::string& local_timestamp_pg_time_zone_name)
{
    ...
    info_contact_query(
            ...
            "WHERE cobr.type=get_object_type_id('contact')");
    ...
    info_contact_query(" ORDER BY ")(GetAlias::historyid())(" DESC");
        ...
        const std::string& local_timestamp_pg_time_zone_name)
{
    ...
    for (Database::Result::size_type i = 0; i < query_result.size(); ++i)
    {
        ...
        info_contact_output.info_contact_data.uuid = query_result[i][GetAlias::uuid()].as<RegistrableObject::Contact::ContactUuid>();
        ...
        info_contact_output.info_contact_data.history_uuid = query_result[i][GetAlias::history_uuid()].as<RegistrableObject::Contact::ContactHistoryUuid>();
