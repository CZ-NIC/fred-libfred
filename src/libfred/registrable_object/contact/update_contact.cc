...
update<ContactAddressType::MAILING>(const ContactAddress&);

template ContactAddressToUpdate& ContactAddressToUpdate::
update<ContactAddressType::BILLING>(const ContactAddress&);

template ContactAddressToUpdate& ContactAddressToUpdate::
update<ContactAddressType::SHIPPING>(const ContactAddress&);

template ContactAddressToUpdate& ContactAddressToUpdate::
update<ContactAddressType::SHIPPING_2>(const ContactAddress&);

template ContactAddressToUpdate& ContactAddressToUpdate::
update<ContactAddressType::SHIPPING_3>(const ContactAddress&);
...
UpdateContact<DERIVED>::UpdateContact(
...
unsigned long long UpdateContact<DERIVED>::exec(OperationContext& ctx, const InfoContactOutput& contact)
{
    ...
    try
    {
        history_id = UpdateObject(
                contact.info_contact_data.handle,
                "contact",
                registrar_,
                authinfo_,
    ...
    //update contact
    {
        ...
        if (place_.isset())
        {
            ...
            {
                const LibFred::Contact::PlaceAddress place = place_.get_value().get_value();
        ...
        if (personal_id_.isset())
        {
            ...
            {
                update_set << set_separator.get() << "ssntype=NULL::integer,ssn=NULL::text";
            }
            else
            {
                const PersonalIdUnion personal_id = nullable_personal_id.get_value();
                const ::size_t ssn_type_id(
                        Contact::get_ssntype_id(
                                personal_id.get_type(),
                                ctx,
                                &update_contact_exception,
                                &Exception::set_unknown_ssntype));
        ...
        if (!update_set.str().empty())
        {
            ...
            Contact::Verification::conditionally_cancel_final_states(
                ctx,
        ...
        if (!addresses_.to_update().empty())
        {
                 ...
                 addr_ptr != to_update.end(); ++addr_ptr)
            {
                ...
                if (updatable.find(addr_ptr->first) != updatable.end())
                { //UPDATE
                    std::ostringstream sql;
                    ...
                    {
                        if ((addr_ptr->first.value != LibFred::ContactAddressType::SHIPPING) &&
                            (addr_ptr->first.value != LibFred::ContactAddressType::SHIPPING_2) &&
                            (addr_ptr->first.value != LibFred::ContactAddressType::SHIPPING_3))
                        {
...
unsigned long long UpdateContactById::exec(LibFred::OperationContext& ctx)
{
    try
    {
        ...
        LibFred::InfoContactOutput contact;//contact selected for update
        try
        {
        ...
        catch (const LibFred::InfoContactById::Exception& e)
        {
            ...
            {
                update_exception.set_unknown_contact_id(e.get_unknown_object_id());
            }
            else
            {
                throw;//rethrow unexpected
        ...
        catch (const UpdateContact<UpdateContactById>::Exception& e)
        {
            if (e.is_set_unknown_registrar_handle())
            {
                //fatal good path, need valid registrar performing update
                BOOST_THROW_EXCEPTION(ExceptionType()
                                      .set_unknown_registrar_handle(e.get_unknown_registrar_handle()));
            }
            ...
            {
                update_exception.set_unknown_ssntype(e.get_unknown_ssntype());
                caught_exception_has_been_handled = true;
            }
            if (e.is_set_unknown_country())
            {
                update_exception.set_unknown_country(e.get_unknown_country());
                caught_exception_has_been_handled = true;
            }
            if (e.is_set_forbidden_company_name_setting())
            {
                update_exception.set_forbidden_company_name_setting(e.get_forbidden_company_name_setting());
                caught_exception_has_been_handled = true;
            }
...
std::string UpdateContactById::to_string() const
{
    return Util::format_operation_state(
            ...
            Util::vector_of<std::pair<std::string, std::string>>
                ...
                (std::make_pair("UpdateContact<UpdateContactById>",
                                UpdateContact<UpdateContactById>::to_string())));//parent operation
}
...
UpdateContactByHandle::UpdateContactByHandle(
        ...
        const Optional<Nullable<bool>>& domain_expiration_warning_letter_enabled,
        const Optional<unsigned long long>& logd_request_id)
    : UpdateContact<UpdateContactByHandle>(
            registrar,
            authinfo,
            name,
            organization,
...
unsigned long long UpdateContactByHandle::exec(LibFred::OperationContext& ctx)
{
    ...
    {
        ExceptionType update_exception;
        LibFred::InfoContactOutput contact;
        try
        {
        ...
        catch (const LibFred::InfoContactByHandle::Exception& e)
        {
            if (e.is_set_unknown_contact_handle())
            {
                update_exception.set_unknown_contact_handle(e.get_unknown_contact_handle());
            }
            else
            {
                throw;//rethrow unexpected
        ...
        catch (const UpdateContact<UpdateContactByHandle>::Exception& e)
        {
            if (e.is_set_unknown_registrar_handle())
            {
                //fatal good path, need valid registrar performing update
...
std::string UpdateContactByHandle::to_string() const
{
    return Util::format_operation_state(
            ...
            Util::vector_of<std::pair<std::string, std::string>>
                ...
                (std::make_pair("UpdateContact<UpdateContactByHandle>",
                                UpdateContact<UpdateContactByHandle>::to_string())));//parent operation
}
