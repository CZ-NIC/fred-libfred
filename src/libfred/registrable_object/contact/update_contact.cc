            ...
            for (ContactAddressToUpdate::ToUpdate::const_iterator addr_ptr = to_update.begin();
                ...
                if (updatable.find(addr_ptr->first) != updatable.end()) { //UPDATE
                    ...
                    if (address.company_name.isset()) {
                        if (type != LibFred::ContactAddressType::to_string(LibFred::ContactAddressType::SHIPPING)
                                && type != LibFred::ContactAddressType::to_string(LibFred::ContactAddressType::SHIPPING_2)
                                && type != LibFred::ContactAddressType::to_string(LibFred::ContactAddressType::SHIPPING_3))
                        {
