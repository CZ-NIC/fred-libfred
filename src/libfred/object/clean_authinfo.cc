...
int CleanAuthinfo::exec(const OperationContext& ctx) const
{
    try
    {
        ...
        FREDLOG_DEBUG(boost::format{"%1% authinfos was cleaned"} % number_of_cleaned_authinfos);
        return number_of_cleaned_authinfos;
    }
    catch (const std::exception& e)
    {
        FREDLOG_ERROR(boost::format{"Cleaning authinfo of object %1% failed: %2%"} % *object_id_ % e.what());
    }
    catch (...)
    {
        FREDLOG_ERROR(boost::format{"Cleaning authinfo of object %1% failed: unspecified exception caught"} % *object_id_);
    }
