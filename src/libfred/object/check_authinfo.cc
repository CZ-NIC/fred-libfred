        ...
        CheckAuthinfo::Visitor on_match)
{
    ...
    for (std::size_t idx = 0; idx < dbres.size(); ++idx)
    {
        ...
        {
            LOGGER.debug(boost::format{"Password does not match the Authinfo %1% for object %2%, registrar %3%, "
...
int CheckAuthinfo::exec(const OperationContext& ctx, const std::string& plaintext_password, Visitor on_match) const
{
    ...
    {
        LOGGER.error(boost::format{"Check authinfo for object %1% failed: %2%"} % *object_id_ % e.what());
    }
    catch (...)
    {
        LOGGER.error(boost::format{"Check authinfo for object %1% failed: unknown exception caught"} % *object_id_);
    }
...
void CheckAuthinfo::increment_usage(const OperationContext& ctx, const AuthinfoId& authinfo_id)
{
    ...
    if (dbres.size() == 1)
    {
        LOGGER.debug(boost::format{"Authinfo %1% for object %2%, registrar %3%, created at %4% and expires at %5% "
                                    "was used %6% times" } %
                      *authinfo_id %
    ...
    LOGGER.warning(boost::format{"Unable to increment usage counter: authinfo %1% not found" } % *authinfo_id);
}
...
void CheckAuthinfo::increment_usage_and_cancel(const OperationContext& ctx, const AuthinfoId& authinfo_id)
{
    ...
    LOGGER.error(boost::format{"Unable to cancel and to increment usage counter: authinfo %1% not found" } % *authinfo_id);
    throw CheckAuthinfoFailure{};
}
