...
unsigned long long DeleteRegistrarZoneAccess::exec(const OperationContext& ctx) const
{
    try
    {
    ...
    {
        FREDLOG_INFO(boost::format{"Exception caught by delete: %1%"} % e.what());
        throw;
    }
    catch (const std::exception& e)
    {
        FREDLOG_INFO(boost::format{"Unexpected std::exception caught by delete: %1%"} % e.what());
    ...
    {
        FREDLOG_INFO("Unexpected exception caught by delete");
