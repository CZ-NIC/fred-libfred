...
unsigned long long UpdateRegistrarZoneAccess::exec(const OperationContext& _ctx) const
{
    ...
    {
        FREDLOG_INFO(boost::format{"Exception caught by update: %1%"} % e.what());
        throw;
    }
    catch (const std::exception& e)
    {
        FREDLOG_INFO(boost::format{"Unexpected std::exception caught by update: %1%"} % e.what());
    ...
    {
        FREDLOG_INFO("Unexpected exception caught by update");
