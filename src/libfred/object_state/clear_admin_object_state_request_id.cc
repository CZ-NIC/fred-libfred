...
ClearAdminObjectStateRequestId::ClearAdminObjectStateRequestId(
        ObjectId _object_id,
...
void ClearAdminObjectStateRequestId::exec(const OperationContext& _ctx)
{
    try
    {
        const ClearObjectStateRequestId::Requests requests = ClearObjectStateRequestId(object_id_).exec(_ctx);
        if (requests.empty())
        {
        ...
        const Database::Result osr_id_res = _ctx.get_conn().exec_params(query.str(), param);
