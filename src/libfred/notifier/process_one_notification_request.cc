...
void send_email(std::shared_ptr<LibFred::Mailer::Manager> mailer, const EmailData& data)
{
         ...
         address_ptr != trimmed_recipient_email_addresses.end(); ++address_ptr)
    {
        ...
        catch (const LibFred::Mailer::NOT_SEND& e)
        {
...
bool process_one_notification_request(LibFred::OperationContext& _ctx, std::shared_ptr<LibFred::Mailer::Manager> _mailer) {
    ...
    try
    {
        ...
        notification_request request(
            EventOnObject(
                LibFred::object_type_from_db_handle( static_cast<std::string>( notification_to_send_res[0]["object_type_"] ) ),
