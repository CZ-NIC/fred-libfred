...
bpo::options_description make_program_options(AdminDatabase& cfg)
{
    ...
    database_options.add_options()
        ...
        ("admin_database.name", bpo::value<std::string>()->notifier(set_dbname), "the database name")
...
Util::Arguments& push_back(Util::Arguments& args, const AdminDatabase& cfg)
{
    return args.push_back_non_empty("--admin_database.name=", cfg.dbname)
               .push_back_non_empty("--admin_database.host=", cfg.host)
               .push_back_non_empty("--admin_database.password=", cfg.pass)
               .push_back_non_empty("--admin_database.port=", cfg.port)
               .push_back_non_empty("--admin_database.timeout=", cfg.timeout)
               .push_back_non_empty("--admin_database.user=", cfg.user);
}
      ...
      test_database_name_{std::move(test_database_name)}
{
    if (setup)
    {
        ...
        FREDLOG_TRACE("setup fred database start");
        database_setup_procedure();
        FREDLOG_TRACE("setup fred database done");
    }
}
...
DatabaseAdministrator::BackupDatabase::~BackupDatabase()
{
    if (this->has_backup())
    {
        ...
        {
            FREDLOG_ERROR(std::string{"reset failure: "} + e.what());
        }
        catch (...) { }
    }
}
...
void DatabaseAdministrator::BackupDatabase::backup(std::string origin, std::string backup)
{
    ...
    FREDLOG_TRACE("backup " + origin_ + " into " + backup_);
...
{
    FREDLOG_TRACE("recover " + backup_ + " into " + origin_);
...
{
    auto admin_db_conn = get_admin_connection(backup_);
    FREDLOG_TRACE("reset " + backup_ + " into " + origin_);
    drop_database_if_exists(*admin_db_conn, origin_);
    copy_database(*admin_db_conn, backup_, origin_);
...
DatabaseAdministrator handle_command_line_args(const HandlerPtrVector& config_handlers, std::function<void()> database_setup_procedure)
{
    try
    {
        ...
        setup_logging(std::get<Logging>(std::get<ConfigFileOptions>(cfg)));
        {
            ...
            push_back(args, std::get<AdminDatabase>(std::get<ConfigFileOptions>(cfg)));
    ...
    catch (const Generic::Help& h)
    {
        try
        {
            bpo::options_description options_group{boost::unit_test::framework::master_test_suite().argv[0] +
                                                   std::string{" options"}, terminal_width};
            options_group.add(*HandleTestsArgs{""}.get_options_description());
            std::for_each(begin(config_handlers), end(config_handlers), [&](auto&& handler)
            {
                options_group.add(*(handler->get_options_description()));
            });
            std::cout << options_group << std::endl;
            ::_exit(EXIT_SUCCESS);
        }
        catch (...) { }
        ::_exit(EXIT_FAILURE);
