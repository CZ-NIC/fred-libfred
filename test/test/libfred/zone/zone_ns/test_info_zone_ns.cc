...
{
    explicit InfoZoneNsFixture(const ::LibFred::OperationContext& _ctx)
...
BOOST_AUTO_TEST_CASE(set_min_info_zone_ns)
{
    ...
    const ::LibFred::Zone::InfoZoneNsData zone_ns_info = ::LibFred::Zone::InfoZoneNs(info_zone_ns.id).exec(ctx);
...
BOOST_AUTO_TEST_SUITE_END()//TestInfoZoneNs
