...
struct has_keyset : has_autocomitting_ctx
{
          ...
          keyset_handle("KEYSET1")
    {
        ...
        {
            const unsigned long long crhid = Test::exec(
...
class has_keyset_changed_authinfo : public has_keyset
{
private:
...
class has_keyset_removed_tech_c : public has_keyset
{
public:
          ...
          historical_keyset_data(::LibFred::InfoKeysetByHandle(keyset_handle).exec(ctx).info_keyset_data)
    {
        keyset_data_to_be_notified = has_updated_keyset_followed_by_future_changes(
                keyset_handle,
                registrar.handle,
                ::LibFred::UpdateKeyset(keyset_handle,
                                        historical_keyset_data.create_registrar_handle).rem_tech_contact(keyset_tech_c1_to_be_notified.handle),
                ctx).keyset_data_to_be_notified;
    }
    ::LibFred::InfoKeysetData keyset_data_to_be_notified;
private:
...
class has_keyset_added_tech_c : public has_keyset
{
public:
    has_keyset_added_tech_c()
        : has_keyset(),
          added_tech_c(Test::exec(
...
class has_keyset_changed_tech_c : public has_keyset
{
public:
...
BOOST_AUTO_TEST_SUITE_END()//TestNotifier/GatherEmailAddresses/Keyset/Update
