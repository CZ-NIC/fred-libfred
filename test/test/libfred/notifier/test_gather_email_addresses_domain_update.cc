          ...
          keyset(
                  Test::exec(
                          ::LibFred::CreateKeyset("KEYSET1", registrar.handle)
                              .set_tech_contacts({ keyset_tech_c1.handle, keyset_tech_c2.handle }),
                          ctx)),
          nsset_tech_c1(
                  Test::exec(
          ...
          nsset(
                  Test::exec(
                          ::LibFred::CreateNsset("NSSET1", registrar.handle)
                              .set_tech_contacts({ nsset_tech_c1.handle, nsset_tech_c2.handle }),
                          ctx))
    {
            ...
            const unsigned long long crhid =
                Test::exec(
                    ::LibFred::CreateDomain(fqdn, registrar.handle, old_domain_registrant_history_to_be_notified.handle )
                        .set_admin_contacts({ old_admin_c1_history_to_be_notified.handle, old_admin_c2_history_to_be_notified.handle })
...
BOOST_FIXTURE_TEST_CASE(test_updated_domain_authinfo, has_domain_changed_authinfo)
{
    ...
    BOOST_CHECK(
        Notification::gather_email_addresses(
            ...
            dom_data_to_be_notified.historyid) == email_addresses);
}
...
BOOST_FIXTURE_TEST_CASE(test_updated_domain_changed_registrant, has_domain_changed_registrant)
{
    const std::set<std::string> email_addresses =
            {
                previous_registrant.notifyemail.get_value(),
          ...
          new_keyset(
                  Test::exec(
                          ::LibFred::CreateKeyset("NEW_KEYSET", registrar.handle)
                              .set_tech_contacts({ new_keyset_tech_c1.handle, new_keyset_tech_c2.handle }),
                          ctx))
    {
          ...
          new_nsset(
                  Test::exec(
                          ::LibFred::CreateNsset("NEW_NSSET", registrar.handle)
                              .set_tech_contacts({ new_nsset_tech_c1.handle, new_nsset_tech_c2.handle }),
                          ctx))
    {
...
/* TODO (jakmile to bude naimplementovano) tady budou testy pro zmenu nssetu/keysetu a "soucasnou" zmenu jejich technickych kontaktu - aby bylo videt, ze se notifikuji spravne historicke verze*/

BOOST_AUTO_TEST_SUITE_END()//TestNotifier/GatherEmailAddresses/Domain/Update
BOOST_AUTO_TEST_SUITE_END()//TestNotifier/GatherEmailAddresses/Domain
