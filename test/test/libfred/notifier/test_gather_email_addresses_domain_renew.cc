...
struct has_renewed_domain : public has_domain {
    ...
    has_renewed_domain() {
        const unsigned long long to_be_notified_hid =
            ::LibFred::UpdateDomain(fqdn, registrar.handle)
                .set_domain_expiration(
                    ::LibFred::InfoDomainByFqdn(fqdn).exec(ctx).info_domain_data.expiration_date + boost::gregorian::years(3)
                )
                .exec(ctx);
        ...
        const std::string different_registrant_handle = "DIFFREGISTRANT";
        {
            const unsigned long long crhid =
                Test::exec(
                    ::LibFred::CreateContact(different_registrant_handle, registrar.handle)
        ...
        const std::string different_admin_handle = "DIFFADMINC";
        {
            const unsigned long long crhid =
                Test::exec(
                    ::LibFred::CreateContact(different_admin_handle, registrar.handle)
        ...
        make_history_version_end_older( ctx, ::LibFred::InfoDomainByFqdn(fqdn).exec(ctx).info_domain_data.crhistoryid, notification_is_years_ago );
        ...
        dom_data_to_be_notified = ::LibFred::InfoDomainHistoryByHistoryid(to_be_notified_hid).exec(ctx).info_domain_data;
    }
};
