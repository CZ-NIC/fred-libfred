          ...
          new_domain_data(
                  ::LibFred::InfoDomainHistoryByHistoryid(
                          ::LibFred::UpdateDomain(dom.fqdn, dom.create_registrar_handle)
                              ...
                              .exec(ctx)).exec(ctx).info_domain_data)
    { }
};
    ...
    std::map<std::string, std::string> etalon =
            {
                ...
                {"changes.domain.registrant.new", new_domain_data.registrant.handle},

                {"changes.domain.nsset", "1"},
                {"changes.domain.nsset.old", ""},
                {"changes.domain.nsset.new", new_domain_data.nsset.get_value().handle},

                {"changes.domain.keyset", "1"},
                {"changes.domain.keyset.old", ""},
                {"changes.domain.keyset.new", new_domain_data.keyset.get_value().handle}
            };
    ...
    std::map<std::string, std::string> etalon =
            {
                ...
                {"changes.domain.registrant.new", newest_domain_data.registrant.handle},

                {"changes.domain.nsset", "1"},
                {"changes.domain.nsset.old", new_domain_data.nsset.get_value().handle},
                {"changes.domain.nsset.new", newest_domain_data.nsset.get_value().handle},

                {"changes.domain.keyset", "1"},
                {"changes.domain.keyset.old", new_domain_data   .keyset.get_value().handle},
                {"changes.domain.keyset.new", newest_domain_data.keyset.get_value().handle},
...
BOOST_AUTO_TEST_SUITE_END()//TestNotifier/GatherEmailContent/Domain/Update
