...
BOOST_AUTO_TEST_CASE(test_base64)
{
    ...
    for (int idx = 0; idx < 5; ++idx)
    {
        ...
        auto data = std::unique_ptr<unsigned char[]>(new unsigned char[data_len]);
        std::memcpy(data.get(), src_data[idx], data_len);
        const auto src = ::PasswordStorage::BinaryData::from_raw_binary_data(std::move(data), data_len);
        ...
        {
            BOOST_CHECK_EQUAL(std::memcmp(src.get_raw_binary_data(), result.get_raw_binary_data(), data_len), 0);
        }
    }
}
