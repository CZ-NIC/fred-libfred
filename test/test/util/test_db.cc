...
template <typename T, std::size_t R, std::size_t C>
std::string make_copy_input(const Table<T, R, C>& table, const PgCopyParams& params)
{
    std::string aux;
...
template <typename T, std::size_t R, std::size_t C>
void check_result(const Database::Result& result, const Table<T, R, C>& table, const PgCopyParams& params)
{
