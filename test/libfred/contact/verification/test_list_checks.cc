...
BOOST_AUTO_TEST_CASE(test_Exec_testsuite_filter)
{
    ...
    {
        if (handles_to_be_listed.find(item.check_handle) == handles_to_be_listed.end())
        {
            BOOST_FAIL("Invalid check listed.");
            break;
        }
        handles_to_be_listed.find(item.check_handle)->second = true;
    }

    for (const auto& item : handles_to_be_listed)
    {
        if (!item.second)
        {
...
BOOST_AUTO_TEST_CASE(test_Exec_contact_filter)
{
    ...
    std::string the_check_handle = ::LibFred::CreateContactCheck(
            ...
            testsuite.testsuite_handle).exec(ctx);

    const std::vector<::LibFred::ListChecksItem> list = ::LibFred::ListContactChecks()
            .set_max_item_count(100)
...
BOOST_AUTO_TEST_SUITE_END()//TestContactVerification/TestListContactChecks_integ
