        ...
        const std::string& database_name_disable_connection)
{
    ...
    {
        const Database::Result dbres = conn->exec(
                "SELECT column_name "
    ...
    const std::string query =
            ...
            "WHERE usename='fred' OR (datname=$1::text AND " + pid_column_name + "!=pg_backend_pid())";
...
void force_drop_db(const std::string& db_name, const std::unique_ptr<Database::StandaloneConnection>& conn)
{
...
{
     auto db_opts = std::make_shared<po::options_description>(std::string("Admin database connection configuration"));
